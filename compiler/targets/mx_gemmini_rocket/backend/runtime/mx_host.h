...
//
// PORTED from gemmini-rocc-tests/include/mx_host.h (merlin_glue_port_plan.md D1/D5, Step 6a). This
// is the backend's RUNTIME: the emitted driver #includes it, and runner.py puts this directory on
// the include path. It is workload-side code, so it lives here rather than in the reference tree;
// only `gemmini.h` (the accelerator's ABI) stays external.
//
// Every routine here has a Python twin used for the reference and the golden, and the two agreeing
// is a TEST, not an assumption -- tests/selftest_mx_host.py. The expectation is zero mismatches
// because E4M3 keeps 3 mantissa bits and absorbs a last-ulp difference between newlib and numpy.
//
...
// Row softmax over a [M][N] bf16 tile, in fp32. `causal` masks j > m (attention over a lower
// triangle, which requires N == M); 0 softmaxes the whole row. Max-subtracted for stability, over
// exactly the elements that survive the mask -- masking after the max would change the result.
//
// `mx_softmax_causal` below is the M x M causal wrapper, kept so its callers are unchanged.
static void mx_softmax_rows(const uint16_t *s_bf16, int M, int N, float scale, int causal,
                            float *out) {
  for (int m = 0; m < M; m++) {
    const uint16_t *row = s_bf16 + (size_t) m * N;
    float *dst = out + (size_t) m * N;
    const int hi = causal ? (m + 1) : N;               // exclusive upper bound
    float mx = -INFINITY;
    for (int j = 0; j < hi; j++) {
    ...
    float sum = 0.0f;
    for (int j = 0; j < hi; j++) { dst[j] = expf(dst[j] - mx); sum += dst[j]; }
    float inv = 1.0f / sum;
    for (int j = 0; j < hi; j++) dst[j] *= inv;
    for (int j = hi; j < N; j++) dst[j] = 0.0f;
  }
}

static void mx_softmax_causal(const uint16_t *s_bf16, int M, float scale, float *out) {
  mx_softmax_rows(s_bf16, M, M, scale, 1, out);
}

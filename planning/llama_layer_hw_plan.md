...
`results/` -- 457 run records, 45 MB, of which 124 are `linear` and 42 are `attention`. Same
argument for pruning applies, but it holds the graded history (every `VERDICT` line ever produced),
so it is a separate decision from clearing build output.

...
Across recipes (2026-09-25): `linear`, `mlp2`, `mlp3`, `attention` (one graph ELF) and
`attention --per-stage-elf` on `baseline`, `flat_acc4`, `wide_acc` and `narrow_prod` — 20 runs, all
4096/4096 identical at tier `mxquant_recipe_exact`.

**Found and fixed 2026-09-24/25: the functional model accumulated across matmuls.** Since the graph
emitter landed (c01c3a0, 2026-09-11) the pipeline ran the graph ELF and then *also* the per-stage
path, which overwrote the hardware output, so the graph ELF was never graded (and every attention
run cost seven spike runs). Graded, its second matmul came out as Q+K: libgemmini's MX loop read the
